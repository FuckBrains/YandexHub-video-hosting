...
class DonationView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if channel:
            ...
            context['url'] = f'{settings.DOMEN}channel/{channel.user_id}/'
            context['channel'] = channel
            return context
        else:
            # if channel not found
...
class ChannelSettingsView(TemplateView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not file_extension in settings.IMAGE_EXTENSIONS:
                ...
                if _file.size > settings.MAX_IMAGE_SIZE:
                    messages.error(
        ...
        try:
            if request.POST['title'] is None or len(request.POST['title'].replace(' ', '')) == 0:
                ...
                if 'video' not in request.FILES or 'video_banner' not in request.FILES:
                    ...
                    if not file_extension in settings.VIDEO_EXTENSIONS:
                        messages.error(request,
            ...
            if track.creator == self.request.user:
                try:
                    ...
                    if not file_extension in settings.IMAGE_EXTENSIONS:
                        ...
                        if poster.size > settings.MAX_IMAGE_SIZE:
                            messages.error(
        ...
        try:
            if request.POST['title'] is None or len(request.POST['title'].replace(' ', '')) == 0:
                ...
                if 'track' not in request.FILES or 'track_poster' not in request.FILES:
                    ...
                    if not file_extension in settings.TRACK_EXTENSIONS:
                        messages.error(
