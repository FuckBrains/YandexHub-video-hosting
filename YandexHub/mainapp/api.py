...
# VIEWS FUNCS
from .views import YandexHubAlert, DOMEN, MAX_IMAGE_SIZE, MAX_VIDEO_SIZE
...
# LIKES FORMAT
from .templatetags.poll_extras import brief_likes_format
        ...
        if 'track_id' in request.data:
            ...
            if track.count() > 0:
                if track[0].creator == request.user:
                    ...
                    return Response({'data': {}, 'message': 'You are not authorized to delete this track because you are not its creator', 'status': 'err'})
            else:
            ...
            if video.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if like:
                        ...
                        video.creator.save()
                    return Response({'data': {'like': 1, 'dislike': 0, 'stats': {'likes': brief_likes_format(video.likes), 'dislikes': brief_likes_format(video.dislikes)}}, 'message': 'Added to Liked videos 👍', 'status': 'ok'}, status=status.HTTP_200_OK)
                    ...
                    video.creator.all_likes -= 1
                    video.creator.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(video.likes), 'dislikes': brief_likes_format(video.dislikes)}}, 'message': 'Removed from Liked videos 👀', 'status': 'ok'}, status=status.HTTP_200_OK)
            ...
            if video.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if dislike:
                        ...
                        video.creator.all_dislikes += 1
                        video.creator.save()
                    return Response({'data': {'like': 0, 'dislike': 1, 'stats': {'likes': brief_likes_format(video.likes), 'dislikes': brief_likes_format(video.dislikes)}}, 'message': 'You dislike this video 👎', 'status': 'ok'}, status=status.HTTP_200_OK)
                    ...
                    video.creator.all_dislikes -= 1
                    video.creator.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(video.likes), 'dislikes': brief_likes_format(video.dislikes)}}, 'message': 'Dislike removed 😎', 'status': 'ok'}, status=status.HTTP_200_OK)
            ...
            if video.count() > 0:
                ...
                if len(text) < 500:
                    if text.replace(' ', '') != '' and text is not None:
                        ...
                        today = date.today()
                        if Comment.objects.filter(commented_video=video, creator=request.user, date_created_without_time=today).count() < 10:
            ...
            if comment.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if like:
                        comment.likes += 1
                        comment.save()
                    return Response({'data': {'like': 1, 'dislike': 0, 'stats': {'likes': brief_likes_format(comment.likes), 'dislikes': brief_likes_format(comment.dislikes)}}, 'message': '', 'status': 'ok'}, status=status.HTTP_200_OK)

                elif like.count() == 1 and dislike.count() == 0:
                    like[0].delete()
                    comment.likes -= 1
                    comment.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(comment.likes), 'dislikes': brief_likes_format(comment.dislikes)}}, 'message': '', 'status': 'ok'}, status=status.HTTP_200_OK)
            ...
            if comment.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if dislike:
                        comment.dislikes += 1
                        comment.save()
                    return Response({'data': {'like': 0, 'dislike': 1, 'stats': {'likes': brief_likes_format(comment.likes), 'dislikes': brief_likes_format(comment.dislikes)}}, 'message': '', 'status': 'ok'}, status=status.HTTP_200_OK)
                ...
                if comment.count() > 0:
                    ...
                    if len(text) < 500:
                        if text.replace(' ', '') != '' and text is not None:
                            ...
                            today = date.today()
                            if ReplyComment.objects.filter(reply_commented_video=video, comment_parent=comment, creator=request.user, date_created_without_time=today).count() < 10:
        ...
        if 'track_id' in request.data:
            ...
            if track.count() > 0:
                ...
                if request.user == track.creator:
                    ...
                    for i in range(10):
                        day = today - timedelta(days=i)
                        auditions = TrackListenModel.objects.filter(listened_track=track, date_created_without_time__gt=(day))

                        if day >= track.date_created_without_time:
                            auditions_stats[str(day)] = track.auditions - auditions.count()
                            
                    return Response({'data': {'auditions': auditions_stats}, 'message': '', 'status': 'ok'}, status=status.HTTP_200_OK)     
                else:
                    return Response({'data': {}, 'message': 'You do not have permission to view statistics because you are not its author', 'status': 'err'}, status=status.HTTP_200_OK)
            else:
                return Response({'data': {}, 'message': 'Track not found', 'status': 'err'}, status=status.HTTP_200_OK)  
            ...
            if film.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if like:
                        film.likes += 1
                        film.save()
                    return Response({'data': {'like': 1, 'dislike': 0, 'stats': {'likes': brief_likes_format(film.likes), 'dislikes': brief_likes_format(film.dislikes)}}, 'message': 'Your liked this film 🧡', 'status': 'ok'}, status=status.HTTP_200_OK)

                elif like.count() == 1 and dislike.count() == 0:
                    like[0].delete()
                    film.likes -= 1
                    film.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(film.likes), 'dislikes': brief_likes_format(film.dislikes)}}, 'message': 'Like removed 🧸', 'status': 'ok'}, status=status.HTTP_200_OK)
            ...
            if film.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if dislike:
                        film.dislikes += 1
                        film.save()
                    return Response({'data': {'like': 0, 'dislike': 1, 'stats': {'likes': brief_likes_format(film.likes), 'dislikes': brief_likes_format(film.dislikes)}}, 'message': 'Your disliked this film 💔', 'status': 'ok'}, status=status.HTTP_200_OK)

                elif dislike.count() == 1 and like.count() == 0:
                    dislike[0].delete()
                    film.dislikes -= 1
                    film.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(film.likes), 'dislikes': brief_likes_format(film.dislikes)}}, 'message': 'Dislike removed 🥶', 'status': 'ok'}, status=status.HTTP_200_OK)
        ...
        if 'article_id' in request.data:
            ...
            if article.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    return Response({'data': {'like': 1, 'dislike': 0, 'stats': {'likes': brief_likes_format(article.likes), 'dislikes': brief_likes_format(article.dislikes)}}, 'message': 'You like this article 👍', 'status': 'ok'}, status=status.HTTP_200_OK)
                    ...
                    article.creator.all_posts_likes -= 1
                    article.creator.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(article.likes), 'dislikes': brief_likes_format(article.dislikes)}}, 'message': 'Like removed 👀', 'status': 'ok'}, status=status.HTTP_200_OK)
            ...
            if article.count() > 0:
                ...
                if like.count() == 0 and dislike.count() == 0:
                    ...
                    if dislike:
                        ...
                        article.creator.all_posts_dislikes += 1
                        article.creator.save()
                    return Response({'data': {'like': 0, 'dislike': 1, 'stats': {'likes': brief_likes_format(article.likes), 'dislikes': brief_likes_format(article.dislikes)}}, 'message': 'You dislike this article 👎', 'status': 'ok'}, status=status.HTTP_200_OK)
                    ...
                    article.creator.all_posts_dislikes -= 1
                    article.creator.save()
                    return Response({'data': {'like': 0, 'dislike': 0, 'stats': {'likes': brief_likes_format(article.likes), 'dislikes': brief_likes_format(article.dislikes)}}, 'message': 'Dislike removed 😎', 'status': 'ok'}, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        if 'video' in request.data and 'banner' in request.data and 'title' in request.data and 'description' in request.data:
            ...
            if Video.objects.filter(creator=request.user, date_created_without_time=today).count() < 5:
                ...
                return Response({'data': {}, 'message': 'You have exceeded the videos limit for one day today 😥', 'status': 'err'}, status=status.HTTP_200_OK)  
        else:
            return Response({'data': {}, 'status': 'err'}, status=status.HTTP_400_BAD_REQUEST)
...
class ArticleApi(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if article:
            ...
            article_info['creator'] = {'user_id': article.creator.user_id, 'username': article.creator.username}
            article_info['stats'] = {
                'likes': article.likes,
                'dislikes': article.dislikes
            }
        else:
            return Response({'data': {}, 'message': 'Article not found', 'status': 'err'}, status=status.HTTP_400_BAD_REQUEST)
            
        return Response({'data': {'article': article_info}, 'message': message, 'status': 'ok'}, status=status.HTTP_200_OK)
...
class CreateArticleApi(APIView): 
    ...
    def post(self, request):
        if 'text' in request.data:
            text = request.data.get('text')
            if len(text) > 1000000:
                return Response({'data': {}, 'message': 'Maximum text length 1000000 characters.', 'status': 'err'}, status=status.HTTP_200_OK)
            ...
            if Article.objects.filter(creator=request.user, date_created_without_time=today).count() < 10:
                data = {'creator': self.request.user.id, 'text': request.data.get('text')}
                ...
                return Response({'data': {'id': queryset.article_id}, 'message': message, 'status': 'OK'}, status=status.HTTP_200_OK)
            else:
                return Response({'data': {}, 'message': 'You have exceeded the articles limit for one day today 😥', 'status': 'err'}, status=status.HTTP_200_OK)  
        else:
            return Response({'data': {}, 'status': 'err'}, status=status.HTTP_400_BAD_REQUEST)
...
class TracksApi(APIView):
    ...
    def get(self, request):
        ...
        if user_tracks:
            ...
            return Response({'data': {}, 'message': 'Tracks not found', 'status': 'err'}, status=status.HTTP_400_BAD_REQUEST)
            
        return Response({'data': {'tracks': tracks}, 'message': message, 'status': 'ok'}, status=status.HTTP_200_OK)
