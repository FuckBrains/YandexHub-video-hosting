...
def view_func(video, request):
    if request.user.is_authenticated:
        ...
        today = date.today()
        if VideoViewModel.objects.filter(watched_user=request.user, watched_video=video, date_created_without_time=today).count() < 10:
...
def listen_func(track, request):
    if request.user.is_authenticated:
        # maximum 10 views per day (one user)
        today = date.today()
        if TrackListenModel.objects.filter(listening_user=request.user, listened_track=track, date_created_without_time=today).count() < 10:
                ...
                if len(str(wallet)) == 16:
                    if len(str(title)) <= 60:
                        ...
                        if request.user.telegram:
                            # send alert
                            message = YandexHubAlert(f'Donations via YooMoney are successfully connected 🔮\n\n{get_city_and_country_ip(request)}', request.user.telegram)
                            if message == 200:
                                pass 
                            elif message == 400:
...
class AboutView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if channel:
            ...
            # find notification model
            if self.request.user.is_authenticated:
                notifications = Notification.objects.filter(notification_channel=channel, notification_user=self.request.user)
            else:
                notifications = None
    ...
    def post(self, request):
        ...
        try:
            ...
            if not file_extension in IMAGE_EXTENSIONS:
                messages.error(request, 'This file extension is not supported (<b>avatar</b>) 😑<br/>You can read more about supported extensions <a href="/faq/">here</a>.')
        ...
        try:    
            ...
            file_extension = Path(str(_file)).suffix
            if not file_extension in IMAGE_EXTENSIONS:
                messages.error(request, 'This file extension is not supported (<b>banner</b>) 😑<br/>You can read more about supported extensions <a href="/faq/">here</a>.')
                return redirect('channel__settings__page')
            else:
                if _file.size > MAX_IMAGE_SIZE:
                    messages.error(request, 'The size of the photo should not exceed 7.5 MB. 💡')
            ...
            if request.POST['title'] is None or len(request.POST['title'].replace(' ', '')) == 0:
                ...
                if 'video' not in request.FILES or 'video_banner' not in request.FILES:
                    ...
                    if not file_extension in VIDEO_EXTENSIONS:
                        messages.error(request, 'This file extension is not supported (<b>video</b>) 😑<br/>You can read more about supported extensions <a href="/faq/">here</a>.')
                        return redirect('create__video__page')
                    else:
            ...
            if video.creator == self.request.user:
                try:
                    ...
                    if not file_extension in IMAGE_EXTENSIONS:
                        ...
                        if banner.size > MAX_IMAGE_SIZE:
                            messages.error(request, 'The size of the photo should not exceed 7.5 MB. 💡<br/>You can read more about images size <a href="/faq/">here</a>.')
                            return redirect('edit__video__page', video.video_id)
                        else:
            ...
            if len(str(telegram)) != 0:
                if len(str(telegram)) < 25:
                    ...
                    if message == 200:
                        ...
                        messages.error(request, '''An error occurred while sending the notification. Check the correctness of your Telegram ID by the <a style='cursor: pointer; color: #0D6EFD;' onclick='transition_link("/bot/manual/")'>link</a> 🤖''')
                        return redirect('bot__page')
                    else:
                        messages.error(request, '''An error occurred while sending a notification. You have blocked the bot from sending messages to fix this read the <a style='cursor: pointer; color: #0D6EFD;' onclick='transition_link("/bot/manual/")'>manual</a> 🤖''')
                        return redirect('bot__page')
            else:
        ...
        try:
            ...
            if track.creator == self.request.user:
                try:
                    ...
                    if not file_extension in IMAGE_EXTENSIONS:
                        messages.error(request, 'This file extension is not supported (<b>poster</b>) 😑<br/>You can read more about supported extensions <a href="/faq/">here</a>.')
            ...
            if request.POST['title'] is None or len(request.POST['title'].replace(' ', '')) == 0:
                ...
                if 'track' not in request.FILES or 'track_poster' not in request.FILES:
                    ...
                    if not file_extension in TRACK_EXTENSIONS:
                        messages.error(request, 'This file extension is not supported (<b>track</b>) 😑<br/>You can read more about supported extensions <a href="/faq/">here</a>.')
                        return redirect('upload__track__page')
                    ...
                    if Track.objects.filter(creator=request.user, date_created_without_time=today).count() < 5:
                        # create track model
                        track = Track.objects.create(creator=request.user, track=track, track_id=generate_id(32), track_poster=poster, title=title)
                        track.save()
