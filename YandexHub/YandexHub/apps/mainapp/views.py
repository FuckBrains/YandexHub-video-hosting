...
class DonationView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = f'Support - {self.channel.username}'
        context['text'] = self.channel.donat_text
        context['wallet'] = self.channel.wallet
        ...
        if len(str(telegram)) != 0:
            if len(str(telegram)) < 25:
                ...
                if message == 200:
                    ...
                    messages.error(self.request, '''An error occurred while sending the notification. Check the correctness of your Telegram ID by the <a style='cursor: pointer; color: #0D6EFD;' onclick='transition_link("/bot/manual/")'>link</a> 🤖''')
                    return redirect('bot__page')
                else:
                    messages.error(self.request, '''An error occurred while sending a notification. You have blocked the bot from sending messages to fix this read the <a style='cursor: pointer; color: #0D6EFD;' onclick='transition_link("/bot/manual/")'>manual</a> 🤖''')
                    return redirect('bot__page')
